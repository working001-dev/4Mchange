...
<script type="text/javascript" local-section="true">
    ...
    $(document).ready(function(){
        gettingtbData("'<?=date('Y-m-01 00:00:00')?>' and '<?=date('Y-m-t 23:59:59')?>'")
        ...
        $("#id-date-picker-1").on("change", function(){
            let _a = $(this).val();
            let _s = moment().month(_a).startOf('month').format("YYYY-MM-DD 00:00:00");
            let _e = moment().month(_a).endOf('month').format("YYYY-MM-DD 23:59:59");
    ...
    async function ExportToExcel() {
        ...
        let _e = moment().month(_a).endOf('month').format("YYYY-MM-DD 23:59:59");
        let _m = `'${_s}' and '${_e}'`;
        window.open(`<?=base_url()?>reports/exportmonthly_report?m=${_m}&p=${_p}`);
