...
$excel2 = $excel2->load(dirname(__FILE__, 4).'/excel_report/template/tmp_report.xlsx');
//change it
$excel2->setActiveSheetIndex(0);
for($i = 0 ; $i < count($data); $i++){
    $row = $i + 4;
    $excel2->getActiveSheet()->setCellValue("A{$row}", $i+1);
    $excel2->getActiveSheet()->setCellValue("B{$row}", $data[$i]->date);
    $excel2->getActiveSheet()->setCellValue("C{$row}", $data[$i]->pd);
    $excel2->getActiveSheet()->setCellValue("D{$row}", $data[$i]->fourm_number);
    $excel2->getActiveSheet()->setCellValue("E{$row}", $data[$i]->lineName);
    $excel2->getActiveSheet()->setCellValue("F{$row}", $data[$i]->processName);
    $excel2->getActiveSheet()->setCellValue("G{$row}", $data[$i]->rel_production);
    $excel2->getActiveSheet()->setCellValue("H{$row}", $data[$i]->changeTypeCode);
    $excel2->getActiveSheet()->setCellValue("I{$row}", $data[$i]->description);
    $excel2->getActiveSheet()->setCellValue("J{$row}", $data[$i]->permanent);
    $excel2->getActiveSheet()->setCellValue("K{$row}", $data[$i]->temporary);
    $excel2->getActiveSheet()->setCellValue("L{$row}", $data[$i]->start);
    $excel2->getActiveSheet()->setCellValue("M{$row}", $data[$i]->duaDate);
    $excel2->getActiveSheet()->setCellValue("N{$row}", $data[$i]->status); 

    foreach ( range('A', 'N') as $cm ) if($cm != "J" && $cm != "K") $excel2->getActiveSheet()->getColumnDimension($cm)->setAutoSize(true);  
}
