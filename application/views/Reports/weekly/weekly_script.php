...
<script type="text/javascript" local-section="true">
    ...
    $(document).ready(function(){
        gettingtbData("'<?=date('Y-m-d 00:00:00', strtotime("-7 days"))?>' and '<?=date('Y-m-d 23:59:59')?>'")
        ...
        $("#id-date-picker-1").on("change", function(){
            let _a = $(this).val();
            let _s = moment(_a).subtract(7, "days").format("YYYY-MM-DD 00:00:00");
    ...
    async function ExportToExcel() {
        ...
        let _e = moment(_a).format("YYYY-MM-DD 23:59:59");
        let _m = `'${_s}' and '${_e}'`;
        window.open(`<?=base_url()?>reports/exportweekly_report?m=${_m}&p=${_p}`);
